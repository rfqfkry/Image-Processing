...
if __name__ == "__main__":
    ...
    # Load calibration results from numpy file
    calibration_data = np.load('stereo_calibration.npz')
    mtxL  = calibration_data['mtxL']
    distL = calibration_data['distL']
    mtxR  = calibration_data['mtxR']
    ...
    while True:
        ...
        newcameramtxL, roiL = cv.getOptimalNewCameraMatrix(mtxL, distL, (w1, h1), 1, (w1, h1))
        newcameramtxR, roiR = cv.getOptimalNewCameraMatrix(mtxR, distR, (w2, h2), 1, (w2, h2))

        dstL = cv.undistort(l_img, mtxL, distL, None, newcameramtxL)
        dstR = cv.undistort(r_img, mtxR, distR, None, newcameramtxR)
