...
if __name__ == "__main__":
    ...
    capture_device = cv.VideoCapture(1)
    ...
    while True:
        ...
        # disparity 0-255 grayscale map
        # Find all contours in the image
        contours, _ = cv.findContours(disparity, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        ...
        disparity = cv.bitwise_and(disparity, mask_filled)
        ...
        _, disparity_thresholded = cv.threshold(disparity, threshold_value, 255, cv.THRESH_BINARY) #cari dikelas threshold kenapa dia ada 4 argumen
        ...
        disparity_filtered = cv.bilateralFilter(disparity_thresholded, d=9, sigmaColor=75, sigmaSpace=75) #cari dikelas bilateral 
       ...
       #membandingkan dimensi
        if len(rl_img.shape) == 2:  # If image is grayscale
            rl_img = cv.cvtColor(left_resized, cv.COLOR_GRAY2BGR)
        if len(disparity_thresholded_resized.shape) == 2:#jika dia dimensinya 2, maka ditambahkan colorbgr jadilah 3d 
            disparity_thresholded_resized = cv.cvtColor(disparity_thresholded_resized, cv.COLOR_GRAY2BGR)
