...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Convert the images to HSV color space for color-based masking
        hsv_l_img = cv.cvtColor(l_img, cv.COLOR_BGR2HSV)
        hsv_r_img = cv.cvtColor(r_img, cv.COLOR_BGR2HSV)

        # Define the blue color range for masking
        lower_blue = np.array([100, 150, 0])
        upper_blue = np.array([140, 255, 255])

        # Create masks for blue color in both images
        blue_mask_l = cv.inRange(hsv_l_img, lower_blue, upper_blue)
        blue_mask_r = cv.inRange(hsv_r_img, lower_blue, upper_blue)
        ...
        # Invert the blue mask to get the areas outside the blue regions
        blue_mask_l_inv = cv.bitwise_not(blue_mask_l)
        
        # Apply the inverted blue mask to the colored disparity image
        masked_disparity = cv.bitwise_and(colormap_disparity, colormap_disparity, mask=blue_mask_l_inv)
        ...
        rd_masked_disparity = cv.resize(masked_disparity, (512, 512))
        ...
        cv.imshow("Masked Disparity (Outside Blue)", rd_masked_disparity)  # Display the masked disparity image
